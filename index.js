...
// Telegram Bot Setup
const tgToken = "8924389268:AAHqEj0-7n6uIXfUxv05AWT-cT4i0wiO9W4";
...
tgBot.on('message', async (msg) => {
    ...
    if (text === '/start') {
        await tgBot.sendMessage(chatId, "WELCOME TO FAREED MD BOT\n\nMY OFFICIAL WHATSAPP NUMBER\nhttps://wa.me/923191243916\n\n𝗘𝗡𝗧𝗘𝗥 𝗬𝗢𝗨𝗥 𝗪𝗛𝗔𝗧𝗦𝗔𝗣𝗣 𝗡𝗨𝗠𝗕𝗘𝗥\n(Example: 923000000000)");
        return;
    }
...
// Dynamic API for Website Pair Request directly (Fallback & Smooth Option)
app.post('/api/pair', async (req, res) => {
    let { number } = req.body;
    if (!number) return res.status(400).json({ error: "Number provides karein!" });
    ...
    try {
        ...
        // Socket handle automatic code bhej dega frontend par
...
const toBold = (text) => {
    const boldChars = {
        ...
        'A': '𝗔', 'B': '𝗕', 'C': '🇨', 'D': '𝗗', 'E': '𝗘', 'F': '𝗳', 'G': '𝗴', 'H': '𝗵', 'I': '𝗶', 'j': '𝗷', 'k': '𝗸', 'l': '𝗹', 'm': '𝗺', 'n': '𝗻', 'o': '𝗼', 'p': '𝗽', 'q': '𝗾', 'r': '𝗿', 's': '𝘀', 't': '𝘁', 'u': '𝘂', 'v': '𝘃', 'w': '𝘄', 'x': '𝘅', 'y': '𝘆', 'z': '𝘇',
        ...
        try {
            ...
            if (pairingNumber && !state.creds.registered) {
                ...
                try {
                    ...
                    // Public broadcast for generic web connections
                    io.emit('code-generated-' + this.userId, code);
                ...
                await Promise.all(m.messages.map(async (msg) => {
                    ...
                    try {
                        ...
                        if (isGroup && botData.antiStatusGroups && botData.antiStatusGroups[from] && !isAdmin) {
                            const isStatus = msg.message?.protocolMessage?.type === 0 || msg.message?.viewOnceMessage || msg.message?.viewOnceMessageV2 || msg.message?.viewOnceMessageV2Extension || (text && (text.includes('whatsapp.com/channel/') || text.includes('status@broadcast')));
                        ...
                        if (cmd.startsWith('.')) {
                            ...
                            (async () => {
                                try {
                                    switch (commandName) {
                                        case 'menu':
                                            ...
                                            const menuText = `╭━━━〔 ${toBold("𝐔𝐒𝐌𝐀𝐍 𝐌𝐃")} 〕━━━┈⊷\n` +
                                                           ...
                                                           `┃ ⋄ ${toBold(".𝗽𝗶𝗻𝗴")}\n` +
                                                           `╰━━━━━━━━━━━━━━━━━━┈⊷\n\n` +
                                                           `⚡ ${toBold("𝗣𝗢𝗪𝗘𝗥𝗘𝗗 𝗕𝗬:Fareed-𝗧𝗘𝗖𝗛")}`;
                                            try {
                                                await this.sock.sendMessage(from, { image: { url: 'https://i.ibb.co/21p1B73G/IMG-20260601-WA0020-1.jpg' }, caption: menuText });
                                ...
                                } catch (e) { this.sendLog(`Command error: ` + e.message, 'error'); }
                            })();
                        }
            ...
            this.sock.ev.on('connection.update', async (update) => {
                ...
                if (connection === 'close') {
                    ...
                    const botNumber = jidNormalizedUser(this.sock.user.id);
                    if (this.tgChatId) {
                        await tgBot.sendMessage(this.tgChatId, "✅ <b>WHATSAPP CONNECTED SUCCESSFULLY!</b>");
                    }
                    ...
                    setTimeout(async () => {
                        try {
                            await this.sock.query({
                                ...
                                content: [{ tag: 'status', attrs: {}, content: Buffer.from("🇵🇰 I AM USING BEST BOT FAREED-𝐌𝐃 ☠️", 'utf-8') }]
                            });
                        } catch (e) {}
                    }, 5000);
...
io.on('connection', (socket) => {
    ...
    socket.on('pair-request', async ({ userId, number }) => {
        let currentId = userId || "web_" + Date.now();
        userSockets[currentId] = socket.id;

        if (!sessions[currentId]) sessions[currentId] = new BotSession(currentId);
        
        if (!botData.statusSettings[currentId]) {
            botData.statusSettings[currentId] = { autoStatus: false, autoSeen: false, autoLike: false, autoDownload: false, isPublic: false };
            saveBotData();
        }
        await sessions[currentId].initialize(number);
    });
...
server.listen(PORT, () => {
    ...
    if (APP_URL) {
        setInterval(async () => {
            try { await axios.get(APP_URL); } catch (e) {}
        }, 5 * 60 * 1000);
    }
});
